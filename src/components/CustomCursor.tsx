  ...
  return (
    <>
      <div ref={dotRef} className="fixed top-0 left-0 pointer-events-none z-[99999] rounded-full"
        style={{ width: isHovering ? 10 : 6, height: isHovering ? 10 : 6, backgroundColor: '#0F172A', opacity: visible ? 1 : 0, transition: 'width 0.18s ease, height 0.18s ease, opacity 0.2s ease', willChange: 'transform' }}
      />
      <div ref={ringRef} className="fixed top-0 left-0 pointer-events-none z-[99998] rounded-full"
        style={{ width: isHovering ? 44 : 32, height: isHovering ? 44 : 32, border: `1.5px solid ${isHovering ? 'rgba(15,23,42,0.7)' : 'rgba(15,23,42,0.35)'}`, backgroundColor: isHovering ? 'rgba(15,23,42,0.05)' : 'transparent', opacity: visible ? 1 : 0, transition: 'width 0.22s ease, height 0.22s ease, border-color 0.2s ease, background-color 0.2s ease, opacity 0.2s ease', willChange: 'transform' }}
      />
    </>
  );
};

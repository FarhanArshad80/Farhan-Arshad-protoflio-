      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="flex items-center justify-center gap-3">
            ...
            <span className="text-xs font-mono text-[#5a5650] tracking-widest uppercase">Featured Work</span>
            <div className="h-px w-8 bg-[#f5f0e6]/15" />
          </div>
          ...
          <p className="text-[#8a8680] text-base sm:text-lg">A showcase of real-world MERN stack platforms, NestJS API microservices, e-commerce suites, and real-time workspaces engineered by Farhan Arshad.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {projects.map((project, index) => (
            <motion.div key={project.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-60px' }}
              transition={{ duration: 0.45, delay: index * 0.08, ease: 'easeOut' }} whileHover={{ y: -5 }} onClick={() => setSelectedProject(project)}
              className="group relative flex flex-col rounded-3xl overflow-hidden border shadow-xl cursor-pointer bg-[#161616] border-[#f5f0e6]/[0.07] hover:border-[#b7f34a]/30 hover:shadow-[#b7f34a]/10 transition-all duration-300"
            >
              <div className={`relative h-28 bg-gradient-to-br ${currentTheme.gradientClass} flex items-center justify-center overflow-hidden`}>
                ...
                <FileCode className="w-9 h-9 text-[#0d0d0d]/30" />
              </div>
              ...
              <div className="flex flex-col flex-1 p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-[#f5f0e6]/[0.07]">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setSelectedProject(project); }}
                    className="px-2.5 py-1.5 rounded-lg text-[11px] font-semibold text-[#c8c3b8] bg-[#0d0d0d] border border-[#f5f0e6]/[0.07] hover:border-[#b7f34a]/30 transition-colors"
                  >Details</button>
                </div>
              </div>
            </motion.div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0d0d0d]/80 backdrop-blur-md">
            ...
            >
              <div className="p-6 bg-[#0d0d0d] border-b border-[#f5f0e6]/[0.07] flex items-center justify-between">
                ...
                <button onClick={() => setSelectedProject(null)} className="p-2 rounded-xl bg-[#161616] border border-[#f5f0e6]/[0.07] text-[#8a8680] hover:text-[#f5f0e6]" aria-label="Close">
                  <X className="w-4 h-4" />
                </button>
              </div>
